...
export const FounderCalculatorSection: React.FC<FounderCalculatorSectionProps> = ({ onOpenAddSlackModal }) => {
  ...
  // Illustrative planning model only: 1.5 founder-hours per employee/month + 10 fixed hours.
          ...
          <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="flex flex-col justify-between border-t border-slate-200 p-5 sm:p-8 lg:border-l lg:border-t-0 lg:p-9">
              <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2.5 sm:gap-3"><div className="rounded-xl bg-slate-50 p-3.5 sm:rounded-2xl sm:p-4"><span className="block text-[9px] font-bold uppercase tracking-[0.14em] text-slate-500">Modeled time / month</span><div className="mt-1.5 flex items-baseline gap-1"><span className="font-mono text-2xl font-black tracking-tight text-slate-950 sm:text-4xl">{hoursPerMonth}</span><span className="text-xs font-bold text-rose-600">hrs</span></div></div><div className="min-w-0 rounded-xl bg-slate-50 p-3.5 sm:p-4"><span className="block text-[9px] font-bold uppercase tracking-[0.14em] text-slate-500">Illustrative annual value</span><div className="mt-1.5 min-w-0 overflow-hidden"><span className="block whitespace-nowrap font-mono text-[clamp(1.15rem,5.5vw,2.25rem)] font-black tracking-[-0.04em] text-rose-600" title={`₹${annualValue.toLocaleString()}`}>₹{annualValue.toLocaleString()}</span></div></div></div>
                <div className="mt-2.5 rounded-xl border border-sky-200 bg-sky-50 p-3.5 sm:rounded-2xl sm:p-4"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-sky-800">Planning baseline</p><p className="mt-1 text-[11px] leading-5 text-slate-600 sm:text-xs">Illustrative model assumes 1.5 founder-hours per employee per month plus 10 fixed hours. The result represents the modeled value of time, not measured workload, projected savings or guaranteed ROI.</p></div>
              </div>
