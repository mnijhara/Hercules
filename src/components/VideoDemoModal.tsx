...
const ProductVideo: React.FC = () => {
  const canvasRef=useRef<HTMLCanvasElement>(null); const videoRef=useRef<HTMLVideoElement>(null); const [url,setUrl]=useState<string|null>(null); const [unsupported,setUnsupported]=useState(false);
  useEffect(()=>{const c=canvasRef.current;if(!c)return;const stream=c.captureStream?.(FPS);const Ctor=window.MediaRecorder;if(!stream||!Ctor){setUnsupported(true);return;}const mime=['video/webm;codecs=vp9','video/webm','video/mp4'].find(Ctor.isTypeSupported);if(!mime){setUnsupported(true);return;}const ctx=c.getContext('2d');if(!ctx){setUnsupported(true);return;}const chunks:BlobPart[]=[];const rec=new Ctor(stream,{mimeType:mime,videoBitsPerSecond:2200000});const start=performance.now();let raf=0;const frame=(now:number)=>{const e=Math.min(DURATION,now-start);draw(ctx,e);if(e<DURATION)raf=requestAnimationFrame(frame);};rec.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};rec.onstop=()=>{const next=URL.createObjectURL(new Blob(chunks,{type:mime}));setUrl(next)};rec.start(250);raf=requestAnimationFrame(frame);const timer=window.setTimeout(()=>rec.stop(),DURATION+150);return()=>{cancelAnimationFrame(raf);window.clearTimeout(timer);if(rec.state!=='inactive')rec.stop();stream.getTracks().forEach(track=>track.stop());};},[]);
  useEffect(()=>()=>{if(url)URL.revokeObjectURL(url)},[url]);
  if(unsupported)return <div className="flex aspect-video items-center justify-center rounded-2xl bg-slate-900 p-8 text-center text-sm text-slate-300">This browser cannot generate the product video format. The interactive workflow remains available on the page.</div>;
  return <div className="relative aspect-video overflow-hidden rounded-2xl bg-slate-950">{url?<video ref={videoRef} src={url} className="h-full w-full" controls playsInline muted autoPlay aria-label="Hercules product walkthrough"/>:<><canvas ref={canvasRef} width={W} height={H} className="h-full w-full" aria-hidden="true"/><div className="absolute bottom-0 inset-x-0 bg-slate-950/75 px-4 py-3 text-center text-xs text-slate-300" role="status" aria-live="polite">Preparing the Hercules product video…</div></>}</div>;
};
