...
function drawFilmFrame(ctx: CanvasRenderingContext2D, elapsed: number) {
  ...
  ctx.fillStyle = '#38bdf8'; ctx.fillRect(panelX + 48, HEIGHT - 102, (panelW - 96) * progress, 3);
...
export const HerculesProductVideo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Paint the first frame synchronously so the modal never opens to a blank black box.
    drawFilmFrame(ctx, 0);

    const stream = canvas.captureStream?.(FPS);
    if (!stream || typeof MediaRecorder === 'undefined') { setStatus('unsupported'); return; }
    const mimeType = ['video/webm;codecs=vp9', 'video/webm', 'video/mp4'].find((type) => MediaRecorder.isTypeSupported(type));
    if (!mimeType) { setStatus('unsupported'); return; }
    ...
    const draw = (now: number) => {
      ...
      if (elapsed < DURATION_MS && !disposed) raf = requestAnimationFrame(draw);
    };
    ...
    const timeout = window.setTimeout(() => { if (recorder.state !== 'inactive') recorder.stop(); }, DURATION_MS + 150);
    ...
    <div className="relative aspect-video w-full overflow-hidden rounded-2xl bg-slate-950">
      {videoUrl ? (
        ...
        <>
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0 flex items-center justify-center bg-slate-950/70 px-4 py-3 text-center text-xs text-slate-300 backdrop-blur-sm" role="status" aria-live="polite">
            {status === 'unsupported' ? 'Video playback is not available in this browser. Use the AI workspace demo below.' : 'Hercules product film'}
          </div>
        </>
      )}
    </div>
