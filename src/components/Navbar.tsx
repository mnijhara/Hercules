    ...
    <header className="fixed left-0 right-0 top-0 z-40 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-md">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between lg:h-[72px]">
          <a href="#how-it-works" className="group flex min-w-0 items-center gap-2.5" aria-label="Hercules overview">
          ...
          <nav className="hidden items-center gap-6 text-xs font-semibold uppercase tracking-wider text-slate-600 lg:flex" aria-label="Primary navigation">
      ...
      {mobileMenuOpen && (
        <div id="mobile-navigation" className="space-y-2 border-b border-slate-200 bg-white px-4 pb-4 pt-2 lg:hidden" role="navigation" aria-label="Mobile navigation">
          ...
          <div className="grid gap-2 pt-1">
            <button type="button" onClick={() => { closeMobileMenu(); onOpenVideoDemo(); }} className="flex w-full items-center justify-center gap-2 rounded-full border border-slate-200 bg-slate-100 py-3 text-xs font-semibold text-slate-800"><Play className="h-3.5 w-3.5 fill-sky-600 text-sky-600" /> Watch demo</button>
