...
async function startServer() {
  ...
  app.use((_req, res, next) => {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      res.setHeader('Content-Security-Policy', "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'self'; form-action 'self'; img-src 'self' data: https://images.unsplash.com; media-src 'self' blob: https://github.com https://objects.githubusercontent.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; script-src 'self'; connect-src 'self'");
    }
    next();
  });
  ...
  app.post('/api/leads', async (req, res) => {
    ...
    if (!name || !email) return res.status(400).json({ error: 'Name and email are required' }); if (!/^\S+@\S+\.\S+$/.test(email)) return res.status(400).json({ error: 'Enter a valid email address' }); if (!LEAD_WEBHOOK_URL) return res.status(503).json({ error: 'Lead capture is not configured yet' });
    try { const response = await fetch(LEAD_WEBHOOK_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, phone, company, topic, teamSize, notes, source }), signal: AbortSignal.timeout(10000) }); if (!response.ok) throw new Error(`Lead webhook returned ${response.status}`); return res.status(202).json({ ok: true }); }
    catch (error) { console.error('Lead capture error:', error); return res.status(502).json({ error: 'Lead capture destination is unavailable' }); }
  });
